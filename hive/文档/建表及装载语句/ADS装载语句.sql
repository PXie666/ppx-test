     ...
     (select '2023-01-10'                                         dt,
      ...
      from (select if(org_level = 1, city_for_level1.id, city_for_level2.id)     city_id,
            ...
            from (select org_id,
                         ...
                         trans_finish_dur_sec
                  from dws_trans_org_truck_model_type_trans_finish_1d
                  where dt = '2023-01-10') trans_origin
                     left join
                 (select id,
                         org_level,
                         region_id
                 ...
                 (select id,
                         name
                  from dim_region_full
                  where dt = '2023-01-10') city_for_level2
...
from (select '2023-01-10'     dt,
      ...
      where dt = '2023-01-10') deliver_1d
         full outer join
     (select '2023-01-10'    dt,
             1               recent_days,
             sum(sort_count) sort_count
      from dws_trans_org_sort_1d
      where dt = '2023-01-10') sort_1d
